...
pub async fn handle_create_room(mut req: Request, env: Env) -> Result<Response> {
    // リクエストボディを取得
    ...
    // RoomManager DOのfetchメソッドはurl.pathnameを見るため、/create-roomを指定
    let do_request = Request::new_with_init(
        "http://room-manager/create-room",
