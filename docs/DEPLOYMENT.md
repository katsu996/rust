...
開発環境では、`wrangler.toml`に環境変数を直接記述するか、`.dev.vars`ファイルを使用します。
...
# .dev.vars
# 開発環境用の環境変数
ALLOWED_ORIGINS=http://localhost:3000,http://localhost:8787,http://127.0.0.1:3000,http://127.0.0.1:8787
```
...
本番環境の環境変数は、Cloudflare Dashboard または `wrangler secret` コマンドで設定します。
...
5. Environment Variables セクションで変数を追加
...
`src/handlers/websocket/handle_ws.rs`の`validate_origin`関数で、環境変数から許可されたOriginリストを取得するように実装されています。
...
**原因**: Origin検証で許可されていないOriginからのリクエスト

**解決策**:

1. 環境変数`ALLOWED_ORIGINS`に許可するOriginを追加
2. 開発環境では`localhost`が自動的に許可されます
